...
public class PalindromeLengthPuzzle {
		...
		for (int z = charIndex - 1; z >= 0; z--) {
			...
			while (!(z < 0 && j >= string.length())) {
				if ((z >= 0 && string.charAt(z) == string.charAt(charIndex))
						|| (j < string.length() && string.charAt(j) == string.charAt(charIndex))) {
					if (j < string.length() && string.charAt(j) == string.charAt(charIndex)) {
					...
					if (z >= 0 && j < string.length() && string.charAt(z) == string.charAt(j)) {
						StringBuilder oldString = resultString;
						resultString = new StringBuilder();
