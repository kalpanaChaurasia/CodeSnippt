...
public class TwoSum {
	...
	public static void main(String[] args) {
		...
		// there is another value that equals to
		// target - x
		 ...
		 * 
		 * We reduce the look up time from O(n) to O(1) by trading space for speed. A
		 * hash table is built exactly for this purpose, it supports fast look up in
		 ...
		 * iteration we check if each element's complement (target - nums[i]) exists in
		 * the table. Beware that the complement must not be nums[i] itself!
		 */
